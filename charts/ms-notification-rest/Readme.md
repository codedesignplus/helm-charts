# ms-notification-rest Helm Chart

This chart provides the configuration for deploying the ms-notification-rest microservice in Kubernetes, based on the CodeDesignPlus SDK. It leverages the ms-base chart for best practices and common features such as deployment, service configuration, probes, autoscaling, Vault integration, Istio, and more.

## Structure

- **Chart.yaml**: Chart metadata.
- **values.yaml**: Default and configurable values for ms-notification-rest.
...
```sh
helm upgrade --install ms-notification-rest codedesignplus/ms-notification-rest \
    --namespace <namespace> \
    --set ms-base.vault.token=<vault-token> \
    --set ms-base.virtualService.http[0].route[0].destination.host=ms-notification-rest.<namespace>.svc.cluster.local \
    --create-namespace
```
...
| `image.repository` | Container image | codedesignplus/ms-notification-rest |
...
ms-base:
  ...
  image:
    repository: codedesignplus/ms-notification-rest
    pullPolicy: IfNotPresent
    tag: "latest"

  imagePullSecrets: []
  fullnameOverride: "ms-notification-rest"
  nameOverride: "ms-notification-rest"
  ...
  virtualService: 
    ...
    spec:
      ...
      - name: ms-payments
        ...
        route:
        - destination:
            host: ms-notification-rest.codedesignplus.svc.cluster.local
            port:
              number: 5000
