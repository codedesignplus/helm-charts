# ms-users-rest Helm Chart

This chart provides the configuration for deploying the ms-users-rest microservice in Kubernetes, based on the CodeDesignPlus SDK. It leverages the ms-base chart for best practices and common features such as deployment, service configuration, probes, autoscaling, Vault integration, Istio, and more.

## Structure

- **Chart.yaml**: Chart metadata.
- **values.yaml**: Default and configurable values for ms-users-rest.
...
```sh
helm upgrade --install ms-users-rest codedesignplus/ms-users-rest \
    --namespace <namespace> \
    --set ms-base.vault.token=<vault-token> \
    --set ms-base.virtualService.http[0].route[0].destination.host=ms-users-rest.<namespace>.svc.cluster.local \
    --create-namespace
```
...
| `image.repository` | Container image | codedesignplus/ms-users-rest |
...
ms-base:
  ...
  image:
    repository: codedesignplus/ms-users-rest
    pullPolicy: IfNotPresent
    tag: "latest"

  imagePullSecrets: []
  fullnameOverride: "ms-users-rest"
  nameOverride: "ms-users-rest"
  ...
  virtualService: 
    ...
    - name: ms-users
      ...
      route:
      - destination:
          host: ms-users-rest.codedesignplus.svc.cluster.local
          port:
            number: 5000
